...
function createShoppingList() {
    if ($("#shoppingListName").val().trim().length > 0) {
...
function drawItems() {
    ...
    for (var i = 0; i < currentList.items.length; i++) {
        ...
        var $li = $("<li>").html(currentItem.name)
            .attr("id", "item_" + i);
        var $deleteBtn =
            $("<button onclick='deleteItem(" + currentItem.id + ")'>Usuń</button>").appendTo($li);
        var $checkBtn =
            $("<button onclick='checkItem(" + currentItem.id + ")'>Odznacz</button>").appendTo($li);
